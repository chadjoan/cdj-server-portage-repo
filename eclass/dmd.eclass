...
dmd_src_compile() {
	...
	compile_libraries() {
		...
		emake -C druntime -f posix.mak DMD="../$(dmd_gen_exe_dir)/dmd" MODEL=${MODEL} ${PIC} ${MAKE_MUSL} ${MAKE_LIBBACKTRACE} ${MAKE_DEBUG_SYMBOLS} MANIFEST=

		einfo 'Building Phobos 2...'
		emake -C phobos -f posix.mak DMD="../$(dmd_gen_exe_dir)/dmd" MODEL=${MODEL} ${PIC} CUSTOM_DRUNTIME=1 ${MAKE_MUSL} ${MAKE_LIBBACKTRACE} ${MAKE_DEBUG_SYMBOLS}
	}
...
EOF
		...
		cat > linux/bin${MODEL}/dmd.conf << EOF
[Environment]
DFLAGS=-I${IMPORT_DIR} -L--export-dynamic -defaultlib=phobos2 -fPIC -L-L/${PREFIX}/lib -L-rpath -L/${PREFIX}/lib ${DFLAG_LIBBACKTRACE} ${DFLAG_DEBUG_SYMBOLS}
EOF
	fi
	insinto "etc/dmd"
